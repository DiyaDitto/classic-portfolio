...
function TimelineItem({ exp, index, isLast }) {
  ...
  return (
    <div ref={ref} style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "0 2rem" }}>
      ...
      <div style={{ paddingBottom: isLast ? 0 : "2.5rem" }}>
        <div style={{
          ...
          color: "var(--accent)", letterSpacing: "0.1em", marginBottom: "0.4rem",
        }}>
          {exp.duration}
        </div>
        <div style={{
        ...
        <div style={{
          ...
          fontStyle: "italic", color: "var(--accent)", marginBottom: "0.6rem",
        }}>
          {exp.company}
        </div>
        <p style={{
          fontSize: "0.88rem", color: "var(--muted)",
          lineHeight: 1.75, marginBottom: "0.75rem", maxWidth: "560px",
        }}>
          {exp.description}
        </p>
        <div className="skill-tags">
          {exp.tech.map((t) => <span className="tag" key={t}>{t}</span>)}
        </div>
      </div>
    </div>
  );
